...
**Tolerante a fallos transitorios.** Cada `untag` y cada **lectura del ACR de la que depende la purga** (`show-tags`, `manifest list-metadata`, `manifest show`) se reintentan 3 veces con backoff lineal. Si aun así falla, el tag se anota y el barrido **sigue con el resto**; el job falla al final, con la lista de lo que quedó pendiente y el recuento de lo que sí se limpió. Cinco fallos seguidos cortocircuitan el barrido: si el ACR no responde, reintentar tag por tag sólo consume el timeout de 30 minutos. Antes cualquier error abortaba el job en el acto, y como el barrido es semanal volvía a morir en el mismo punto siete días después — `ObligacionesPorPagar.Radicacion` encadenó cuatro semanas así por fallos de resolución DNS en el runner, con el registry en 495 GB sobre una cuota de 10 GB.
...
**Purga de huérfanos.** Como `untag` no libera bytes, el modo barrido borra por digest los manifests que llevan más de **7 días** sin ningún tag **y** que ningún manifest etiquetado referencia. Esa segunda condición no es opcional: las imágenes se publican como índices OCI multi-plataforma, y los manifests hijos del índice no llevan tag propio — un filtro por `tags == null` los vería como huérfanos y purgarlos rompería el `main-<sha7>` que el índice etiqueta. El margen existía en 30 días para el caso en que un digest importa por sí mismo —Swarm lo fija al desplegar y lo reusa si una tarea necesita re-pull—; con el conjunto desplegado protegido por su tag, un manifest sin tag ya no puede ser el de un servicio vivo, así que el margen sólo tiene que cubrir operaciones en vuelo y baja a 7 días.

La lista de digests referenciados sale de **un solo inventario** (`az acr manifest list-metadata`) leído **después** del bucle de untag —`tags_desc` es de antes y ya no describe la realidad— del que salen las dos mitades de la decisión: los manifests **con** tag, que hay que resolver con `az acr manifest show` para no purgar a sus hijos, y los **sin** tag, que son los candidatos. Al venir del mismo snapshot no hay ventana entre una lectura y la otra.

La resolución va **por digest, no por tag**: varios tags apuntan al mismo índice —en `croxpdeveus2001`, 1.526 tags sobre 729 manifests—, así que resolver por tag duplica las llamadas y con ellas las oportunidades de tropezar con la guarda. Si una falla después de sus 3 intentos (throttling del ACR, red, token vencido a mitad del run), los hijos de ESE índice quedan fuera de la lista y se verían como huérfanos. Un índice que desapareció entre el inventario y su resolución no cuenta como fallo: es una carrera con otro run, no hay índice vivo que proteger y sus hijos son huérfanos de verdad.

Los reintentos existen porque **el ACR Basic falla transitoriamente aun con red sana**: en la limpieza manual de `croxpdeveus2001` fallaron 4 de 1.526 verificaciones con `manifest show` y 4 de 6.718 borrados, y los 8 resolvieron al primer reintento — del orden del 0,2%. Sobre los cientos de lecturas de un barrido completo, esa tasa basta para que la guarda se dispare casi siempre, y entonces el barrido desetiqueta **sin liberar un solo byte** mientras el log dice que corrió. Así llegó `croxpdeveus2001` a 538,57 GB sobre los 10 GB del tier Basic, y hubo que limpiarlo fuera de banda (6.718 huérfanos, hasta 26,1 GB).

El borrado va en **dos fases**: primero se decide qué purgar, secuencialmente y dejando cada descarte explicado en el log; después se borra en paralelo (`-P 4`, por debajo del `-P 6` del recorrido, porque esta fase borra y aquella sólo lee). El borrado era el 85% del tiempo del job —1047 purgas secuenciales a ~1,5 s consumieron 26 de los 30 minutos del timeout en el primer barrido real de Contabilidad, que murió sin terminar—, y como los manifests son independientes entre sí, paralelizarlo no cambia ninguna decisión. Un borrado que falla no pierde nada: el manifest sigue ahí, se avisa con un `::warning::` y el barrido siguiente lo reintenta. Por eso la purga es **fail-closed por repositorio**: si el inventario no se pudo leer, o algún índice no se pudo resolver, el job emite un `::warning::` y **omite la purga en ese repositorio**, dejando el untag de `pr-*` intacto. Se pierde espacio hasta el barrido siguiente; no se pierde una imagen desplegada, que es irrecuperable.

Y como esa omisión significa que ahí el barrido **no liberó bytes**, el resumen del job la dice en voz alta: lista los repositorios que quedaron sin purgar con su motivo, y si el barrido purgó **cero** manifests avisa explícitamente que no se liberó espacio. Un barrido que desetiqueta mucho y purga nada no debe leerse como exitoso.
...
- **Poda de `main-*`: opt-in, y sólo con el conjunto desplegado a la vista.** El caller la habilita declarando `runner_group_prod`. Sin ese input el cleanup se comporta como siempre: los `main-*` no son candidatos y se conservan todos.

  Cuando está activa, el conjunto protegido son los últimos `keep_last_main` (3 por defecto) **más todo lo que los swarms de dev y prod tienen corriendo**, leído con `docker service inspect` en cada runner. Esa exclusión no es un extra: **producción consume el mismo ACR dev (D-ACR) y sólo avanza por `workflow_dispatch` deliberado**, así que queda muy por detrás de dev — en `oxp/Radicación` son 79 deploys a dev por 8 promociones a prod en 30 días, unos 10 `main-*` de distancia. Podar por «últimos N» a secas le borraría a producción la imagen que está corriendo, y el síntoma no aparece hasta el primer re-pull.

  Si la lectura de los swarms no devuelve nada, no se poda ningún `main-*` en esa corrida (fail-closed), y una aserción al final del job verifica tag por tag que todo lo protegido siguiera existiendo.

  Los jobs que leen los swarms emiten `<repo>:<tag>`, no el tag suelto: dos servicios del mismo BC comparten el `main-{sha7}` del commit que los construyó, así que sin el repositorio no se sabe a cuál pertenece. Una guarda de formato falla el job si esa forma cambia, para que la exclusión no se quede en silencio sin proteger nada.
...
- **Barrido semanal:** `cleanup-acr-semanal.yml` corre los lunes 03:00 UTC y quita cualquier `pr-*` cuyo PR cerró hace más de 7 días (red de seguridad por si el cierre no disparó cleanup), poda los `main-*` fuera de los últimos 3 cuando el caller habilitó la poda, y purga los manifests con más de 7 días sin ningún tag que además no sean hijos de un índice etiquetado.
