...
La lista de digests referenciados se arma releyendo los tags **después** del bucle de untag —los que el propio run acaba de quitar ya no resuelven, y contarlos como fallo dejaba la purga fuera de juego en cuanto hubiera un solo untag— y llamando `az acr manifest show` una vez por tag vigente — en el ACR de OxP eso son más de mil llamadas por barrido, con repositorios de hasta 251 tags. Si alguna falla (throttling del ACR, red, token vencido a mitad del run), los hijos de ESE índice quedan fuera de la lista y se verían como huérfanos. Un tag que desapareció entre la relectura y su resolución no cuenta como fallo: es una carrera con otro run y no hay índice que proteger.

El borrado va en **dos fases**: primero se decide qué purgar, secuencialmente y dejando cada descarte explicado en el log; después se borra en paralelo (`-P 4`, por debajo del `-P 6` del recorrido, porque esta fase borra y aquella sólo lee). El borrado era el 85% del tiempo del job —1047 purgas secuenciales a ~1,5 s consumieron 26 de los 30 minutos del timeout en el primer barrido real de Contabilidad, que murió sin terminar—, y como los manifests son independientes entre sí, paralelizarlo no cambia ninguna decisión. Un borrado que falla no pierde nada: el manifest sigue ahí, se avisa con un `::warning::` y el barrido siguiente lo reintenta. Por eso la purga es **fail-closed por repositorio**: si algún tag no se pudo resolver, el job emite un `::warning::` con cuáles fueron y **omite la purga en ese repositorio**, dejando el untag de `pr-*` intacto. Se pierde espacio hasta el barrido siguiente; no se pierde una imagen desplegada, que es irrecuperable.
