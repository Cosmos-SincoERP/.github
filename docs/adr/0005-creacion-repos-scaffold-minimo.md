...
# 0005 — Creación de repos: scaffold mínimo (baseline + seguridad); CI/CD deferido al onboarding

## Contexto y problema

[[0004]] adoptó la creación automatizada de repos y, con ella, el concepto de **arquetipo**:
cada tipo de repo recibía al crearse no solo el baseline de gobernanza sino su **CI/CD
completo** (build/deploy, publicación de paquete, plan/apply de infraestructura, etc.).

Ese CI/CD por arquetipo tiene un defecto de fondo: se genera **a ciegas**, en el momento en
que el repo nace vacío. El flujo de creación no puede descubrir nada del repo (proyectos
reales, Dockerfiles, dependencias) ni conoce los valores reales del bounded context (registro
de imágenes, secretos, grupo de runners, redes). Emite un esqueleto con marcadores y `TODO`s
...
`consumes`/`stack` del manifest); que el flujo de creación además lo renderice al crear es
redundante.

Pregunta de decisión: ¿qué debe producir el flujo de creación al crear un repo, y qué se
defiere a las skills de onboarding (o al equipo)?
...
- **A. Statu quo (CI/CD por arquetipo).** El flujo de creación scaffoldea el CI/CD completo
  de cada tipo (con `TODO`s) y la skill de onboarding lo regenera después. Doble fuente; el
...
- **B. Enriquecer el scaffold.** Portar el descubrimiento (scan de proyectos/Dockerfiles,
  valores del bounded context) al propio flujo de creación. Inviable: el repo nace vacío, no
  hay nada que descubrir en el momento de la creación.
- **C. Minimizar el flujo de creación y retirar el arquetipo.** El flujo deja un único
...
1. **Un solo scaffold para todo repo nuevo: baseline + seguridad.** El flujo de creación
   coloca los workflows de seguridad gestionados y nada más. No hay catálogo de arquetipos ni
...
2. **Todo el CI/CD lo añade el onboarding, no la creación.** Build/deploy, publicación de
   ...
   equipo. El flujo de creación nunca los emite.
...
- ✅ Un solo generador del CI/CD (las skills / el equipo, con contexto real); el flujo de
  creación deja de emitir artefactos incompletos.
...
- ⚠️ **Catálogo retirado.** Se elimina el catálogo de arquetipos (plantillas por tipo). Si en
  ...
  no del flujo de creación.

## Referencias

- [[0004]] — Creación automatizada de repositorios (este ADR retira de su decisión el
